...
def get_article_prompt(calc: dict, seo: dict = None, faq: list = None, example_context: dict = None, intent: str = None, law_ssot_block: str = "", valid_calculators: str = None) -> tuple:
    ...
    system = (
        ...
        "[계산 예시 작성 규칙 — 반드시 준수]\n"
        "- \"계산 예시\" 섹션은 반드시 [검증된 계산 데이터]의 verified_examples 배열 중 하나를 그대로 사용해 작성한다.\n"
        "- 선택한 example의 inputs(입력값)와 result(결과값)를 절대 변경하지 않는다. 임의의 날짜·급여·가격·조건을 새로 만들어 대체하지 않는다.\n"
        "- verified_examples가 여러 개라도 전부 사용할 필요는 없다 — 하나만 정확히 사용하면 충분하다.\n"
        "[계산 예시 결과값 표기 규칙]\n"
        "- 금액 결과는 소수점 이하를 버리고 정수 원 단위로 표기하며, 천 단위마다 쉼표를 포함한다.\n"
        "  예: 4249315.068... → 4,249,315원.\n"
        "- 이는 값의 의미를 바꾸는 것이 아니라 원화 표기 관례(원 미만 단위 없음)를 따르는 것이다.\n"
